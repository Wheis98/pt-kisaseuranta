...
const columns = [
  { id: 'vartio', label: 'Vartio', minWidth: 170 },
  {
    id: 'sarja',
    label: 'Sarja',
    minWidth: 170,
...
const timeDifference = (date1, date2) => {
    ...
    return daysDifference > 0
              ? daysDifference + 'pv, ' + hoursDifference + 'h, ' + minutesDifference + 'min, ' + secondsDifference + 's'
              :  hoursDifference + 'h, ' + minutesDifference + 'min, ' + secondsDifference + 's';
}

const createData = (vartio, sarja, rasti, saapumisaika, poistumisaika) => {
  const aika = poistumisaika ? timeDifference(poistumisaika, saapumisaika) : '-';
  return { vartio, sarja, rasti, saapumisaika, poistumisaika: poistumisaika ? poistumisaika : '-', aika_rastilla: aika };
}
...
const rows = [
  createData('Manaatit', 'sininen', 1, new Date("2026-01-18T09:20:00"), new Date("2026-01-18T10:10:00")),
  createData('Undulaatit', 'punainen', 1, new Date("2026-01-18T09:45:00"), new Date("2026-01-18T10:50:00")),
  createData('Manaatit', 'sininen', 2, new Date("2026-01-18T10:24:00")),
];
...
const EnhancedTableToolbar = (props) => {
  ...
  const handleVartioFilterChange = (event) => onVartioFilterChange(event.target.value);
    ...
    >
      ...
      <Box sx={{ display: "flex", gap: "10px", margin: "10px"}}>
        ...
        <TextField
          ...
          variant="outlined"
          value={vartioFilter}
          onChange={handleVartioFilterChange}
        />
...
EnhancedTableToolbar.propTypes = {
  ...
  onVartioFilterChange: PropTypes.func.isRequired,
  vartioFilter: PropTypes.string.isRequired,
  onClearFilters: PropTypes.func.isRequired
};

const ThroughputTable = () => {
  const [order, setOrder] = React.useState('asc');
  const [orderBy, setOrderBy] = React.useState('vartio');
  ...
  const [vartioFilter, setVartioFilter] = React.useState('');
  ...
  const handleVartioFilterChange = (newFilter) => {
    setVartioFilter(newFilter);
  }
  ...
  return (
    <Paper sx={{ width: '100%', overflow: 'hidden' }}>
      <EnhancedTableToolbar
        ...
        onVartioFilterChange={handleVartioFilterChange}
        vartioFilter={vartioFilter}
          ...
          <TableBody>
            {visibleRows
              ...
              .filter((row) =>
                ...
                row.vartio.toLowerCase().includes(vartioFilter.toLowerCase())
              )
              .map((row) => {
                return (
